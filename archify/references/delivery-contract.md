> Prime edition: this reference describes retained delivery evidence, not permission
> to start tools. Use explicit approved output paths. No preview watcher or automatic
> opener. Real-browser checks require non-root sandboxed Chromium. The generated
> viewer has no remote fonts or remote brand capture. Inspect only trusted generated
> HTML, and reserve all sidecar paths before a run. See SKILL.md for higher-priority
> task boundaries and AUDIT.md for measured versus untested behavior.
...
## Prime preview policy

The live watcher preview is disabled. Use one-shot delivery and local inspection.
Do not start a server, watcher or agent-wakeup loop.
