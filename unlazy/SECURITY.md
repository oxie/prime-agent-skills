# Prime adaptation boundary

The installer is a refusal-only stub; original installer ownership/concurrency claims
below are historical upstream descriptions, NOT supported Prime behavior. No hook is
configured. The retained scan-only hook is fixture-tested only. See references/prime.md
for authorization, outside-root approval storage and evidence limits. No command sandbox,
forced model continuation, automatic CHECK approval or comprehensive descendant-cleanup
guarantee is claimed. Never run these stress tests with the live credential environment.
