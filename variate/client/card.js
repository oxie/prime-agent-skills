...
`;
    ...
    post("/request", { type: "done", params: { set: s.name, n, label } })
      .then(() => {
        const settle = () => {
          ...
          note(agentHere() ? kept : `${kept} \u00b7 resume this design in chat to wrap up`,
    ...
    post("/request", { type: "more", params: { set: s.name, from: atOf(s) ?? null, steer, count: 2 } })
      .then(() => {
        ...
        note(agentHere()
          ...
          : "saved \u00b7 resume this design in chat to process it",
    ...
    if (ts === "offline") {
      ...
      b.onclick = word === "stalled"
        ...
        : word === "drawing"
          ...
          : () => note(here ? "your agent is on it" : "resume this design in chat to process it", { ms: 3200 });
      refs.tail.appendChild(b);
    } else {
  ...
  function openMenu() {
    ...
    menuEl.appendChild(el("div", { class: "mfoot", text: `your agent: ${state?.agent === "on-request" ? "on request" : agentHere() ? "here" : "away"}` }));
    ...
    pickClick = (e) => {
      ...
      post("/request", { type: "vary", params: {
        ...
        .then(() => note(agentHere()
          ...
          : "asked for 4 takes \u00b7 resume this design in chat to process it", { ms: 6000 }))
