...
for(const name of ["archify","hallmark","variate"])assert.equal(loaded.skills.filter(s=>s.name===name).length,1);
console.log(`native loader PASS: ${loaded.skills.length} skills, no diagnostics`);
const fixture=fs.mkdtempSync(path.join(os.tmpdir(),"archify-prime-discovery-"));
...
child.stdout.on("data",chunk=>{buffer+=chunk;let at;
 ...
 if(m.id!=="archify-discovery")continue;
 try{assert.equal(m.success,true,JSON.stringify(m));
   for(const name of ["archify","hallmark","variate"]){const c=m.data.commands.filter(c=>c.name===`skill:${name}`);assert.equal(c.length,1);
...
child.stdin.write(JSON.stringify({id:"archify-discovery",type:"get_commands"})+"\n");
