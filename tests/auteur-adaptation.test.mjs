...
test('exact source identity, source license and adaptation payload set',()=>{
 ...
 for(const [path,hash] of Object.entries(prov.adapted_files))assert.equal(sha(readFileSync(resolve(root,path))),hash,path);
...
test('old owners and unchanged Cinematic UI files retain their actual bytes',()=>{
 ...
 for(const [path,hash] of Object.entries(previous))if(!changed.has(path))assert.equal(sha(readFileSync(resolve(root,path))),hash,path);
 for(const [path,hash] of Object.entries({"hallmark/SKILL.md": "44f66f48f6f367a595653f23e774bd5fcde4ce67c3a10d181f35a10fe1869833", "hallmark/refero-provenance.json": "44df23ec8b2aef3a19510ab72d1e696764ad98e214747aa6ef8b2c3ce14b4f4b", "canvas-effects/SKILL.md": "a6268ba4e5c92eb4caa2feb80d532705d087c548d61f2b62effc30990c3eb1fe"}))assert.equal(sha(readFileSync(resolve(skills,path))),hash,path);
