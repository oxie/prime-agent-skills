...
4. **Launch each ready wave.** Give each leaf only the shared contract, its exact ownership and dependencies, its own ledger, and the bounded four-pass completion rule in `prime.md`. Require a complete atomic artifact and an explicit parent message after completed exit evidence. Open a dispatch wave for the independent `READY` leaves, call the host's native nonblocking launch once per leaf, record every host handle, and seal the wave before the first wait or result read. Follow [dispatch.md](dispatch.md); do not leak unrelated leaf histories.
...
4. **Prime root audit:** reconcile the current request, artifacts, completed exits, manual evidence and integration. No Stop hook enforces continuation in Prime; report incomplete handoffs honestly.
