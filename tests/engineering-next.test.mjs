...
test("source/license mapping and hashes are complete for exactly four additions",()=>{
 ...
 for(const owner of ["engineering-references","hallmark"]){
  ...
  for(const f of [...p.files,...p.licenses])assert.equal(hash(owner==="engineering-references" ? beforeColeam00File(root,`${owner}/${f.path}`) : beforeHuashuFile(root,`${owner}/${f.path}`)),f.sha256,f.path);
