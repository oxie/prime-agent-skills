...
import { atomicWrite, nowIso, readJsonSafe, slug, readSet, readBytes, sha, safePath } from "./core.mjs";
import { withQueueLock } from "./requests.mjs";
...
// every string, coerce every number, and drop the rest. This protects the
// filename shape, not prompt semantics: all page text remains untrusted data.
const capStr = (v, max) => {
...
const position = v => v != null && Number.isSafeInteger(Number(v)) && Number(v) > 0 ? Number(v) : undefined;
...
export function sanitizeParams(type, raw) {
  ...
  if (type === "vary") {
    ...
    const from = position(p.from);
    if (from != null) out.from = from;
  } else if (type === "done") {
    const n = position(p.n);
    if (n != null) out.n = n;
...
export function createRequest(P, type, rawParams = {}) {
  ...
  fs.mkdirSync(safePath(P.ROOT, P.REQ_DONE), { recursive: true });
  const params = sanitizeParams(type, rawParams);
  if ((type === "done" || type === "more") && params.set) {
    const set = readSet(P, params.set);
    if (!set) return { error: "Unknown request set" };
    const n = type === "done" ? params.n : params.from;
    const selected = n == null ? null : set.variants.find(v => v.n === n);
    if (n != null && !selected) return { error: "Unknown selected position" };
    const live = readBytes(set.target);
    if (!live) return { error: "Target missing; cannot snapshot the request" };
    params.liveHash = sha(live);
    if (selected) params.selectedHash = selected.sha;
    if (type === "done" && selected && selected.sha !== params.liveHash)
      return { error: "Selection changed before keep; reload and choose again" };
  }
...
export function queueState(P, { strict = false } = {}) {
  safePath(P.ROOT, P.REQ);
  let files = [];
  try { files = fs.readdirSync(P.REQ); } catch (e) { if (strict && e.code !== "ENOENT") throw e; return { queued: [], working: [] }; }
  const queued = [], working = [];
  for (const f of files.sort()) {
    if (!f.endsWith(".json") && !f.endsWith(".json.working")) continue;
    const j = readJsonSafe(safePath(P.ROOT, path.join(P.REQ, f)));
    if (!j) { if (strict) throw new Error(`Unreadable request ${f}; preserve and reconcile before cleanup`); continue; }
