...
Fetch the listing page with an available HTTP or browser capability. In Prime Agent, use `requests` or `httpx` from `ipython`. Extract every available field:
...
If the HTTP fetch returns incomplete data (stores render client-side), note gaps and
work with what's available. Ask the user to paste missing fields if critical.

### Visual asset assessment

A plain HTTP fetch cannot reliably extract screenshot images or caption text. **Use an available browser to take a screenshot
