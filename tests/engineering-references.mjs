...
const references=[...expected.map(p=>path.basename(p)),"contract-boundaries.md","release-dependencies.md","api-authorization.md","sql-diagnosis.md","telemetry-evidence.md","commerce.md",...mattReferences,...superReferences];
const inventory={
 ".":["LICENSE","SKILL.md","UPSTREAM.md","provenance.json","references","THIRD_PARTY.md","core-provenance.json","licenses","NEXT_SOURCES.md","next-provenance.json","ADDY_SOURCES.md","addy-provenance.json","MATTPOCOCK_SOURCES.md","mattpocock-provenance.json","SUPER_SKILLS_SOURCES.md","super-skills-provenance.json","SUPERPOWERS_SOURCES.md","superpowers-provenance.json"],
 references,
 licenses:["AAS-LICENSE-CONTENT.txt","Apache-2.0.txt","CC-BY-4.0.txt","addyosmani-MIT.txt","operating-kit-MIT.txt","stareezy-1-MIT.txt","mattpocock-MIT.txt","super-skills-MIT.txt","superpowers-MIT.txt"],
};
...
for(const name of superReferences){
 const relative=`references/${name}`;
 assert.equal(hash(read(relative)),superSkills.adapted_files[relative].sha256,relative);
}
...
for(const doc of ["SKILL.md","UPSTREAM.md","MATTPOCOCK_SOURCES.md","SUPER_SKILLS_SOURCES.md","SUPERPOWERS_SOURCES.md",...references.map(name=>`references/${name}`)]){
