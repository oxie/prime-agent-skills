...
5. **Use gates and passes as the effort control.** Finish implementation, expert reread, defect hunt, and low-cost polish. Stop when required outcomes and integration gates have current evidence. Apply at most one low-cost polish pass; repeat only for a failed required gate or a concrete in-scope defect.
