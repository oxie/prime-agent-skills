...
## Running a loop in Prime Agent

Default to a **manual cadence**. The value is the repeatable loop body, not unattended automation. Prepare the loop specification and run it once manually before proposing any recurring execution.

Do not create cron jobs, schedules, heartbeats, retained workers, or background processes unless the user explicitly requests recurring automation after the manual path is validated. If the user does request a Prime Agent heartbeat, load and follow the installed `rlm-heartbeat` skill. Show the cadence, scope, stop conditions, and side effects before activation. Consequential actions such as sending, publishing, spending, account mutation, or personal-data processing remain separately approval-gated even inside an approved loop.
