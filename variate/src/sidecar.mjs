...
import {
  ...
  readBytes, defaultPortFor, nowIso, VERSION, safePath,
} from "./core.mjs";
...
function serveStatic(root, urlPath, res, send) {
  ...
  // A public-looking alias can resolve to .env or .variate/token inside
  // the root. Apply the dotfile rule to the resolved path as well.
  if (path.relative(rootReal, real).split(path.sep).some((s) => s.startsWith("."))) return send(res, 403, "no");
  const ext = path.extname(target).toLowerCase();
  const type = STATIC_MIME[ext] ?? "application/octet-stream";
  if (ext === ".html" || ext === ".htm") {
    return send(res, 200, withCard(fs.readFileSync(real, "utf8")), { "Content-Type": type });
  }
  return send(res, 200, fs.readFileSync(real), { "Content-Type": type });
}

export function startSidecar({ root, port, serve = false }) {
  const P = paths(root);
  fs.mkdirSync(safePath(P.ROOT, P.REQ_DONE), { recursive: true });
  fs.mkdirSync(path.dirname(safePath(P.ROOT, P.HEARTBEAT)), { recursive: true });
  safePath(P.ROOT, P.SERVER_JSON);

  let token = readSafe(safePath(P.ROOT, P.TOKEN))?.trim();
  if (!token || token.length < 16) {
    token = crypto.randomBytes(16).toString("hex");
    fs.writeFileSync(safePath(P.ROOT, P.TOKEN), token + "\n", { mode: 0o600 });
  }
  ...
  function computeState() {
    ...
    return {
      ...
      queued: q.queued,
      working: q.working,
      // Prime handles the queue on user request; no autonomous listener.
      agent: "on-request",
      at: nowIso(),
    };
  }
  ...
  const server = http.createServer(async (req, res) => {
    ...
    try {
      ...
      if (req.method === "POST") {
        ...
        if (p === "/switch") {
          safePath(P.ROOT, P.LAST_SWITCH);
          const out = withQueueLock(P, () => switchTo(P, String(body.set ?? ""), Number(body.to), { force: !!body.force }));
          if (!out.error) {
            // The breadcrumb lets the agent's await tell "the user is flipping
            // right now" from "the round has gone quiet".
            atomicWrite(safePath(P.ROOT, P.LAST_SWITCH), JSON.stringify({
        ...
        if (p === "/shutdown") {
          ...
          // Finish the response before closing its socket. The sidecar may
          // be embedded in a CLI/test process, so never exit that process.
  ...
  return new Promise((resolve, reject) => {
    ...
    const listen = (want, left) => {
      ...
      const onListening = () => {
        ...
        try {
          atomicWrite(safePath(P.ROOT, P.SERVER_JSON), JSON.stringify({ pid: process.pid, port: boundPort, root: P.ROOT, startedAt: started, version: VERSION, serve }, null, 2) + "\n");
        } catch (e) {
          clearInterval(tick);
          clearInterval(ping);
