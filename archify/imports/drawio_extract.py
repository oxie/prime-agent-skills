...
def _decode_utf8(data: bytes, *, url_encoded: bool = False) -> str:
    """Fail closed rather than silently replace source facts during decoding."""
    try:
        text = data.decode("utf-8", "strict")
        return unquote(text, errors="strict") if url_encoded else text
    except UnicodeError:
        _fail("diagram contains invalid UTF-8 (including percent-encoded data)")
...
def _inflate(payload: str) -> str | None:
    ...
    for wbits in (-15, 15, 47):
        try:
            text = _decode_utf8(_decompress_limited(raw, wbits), url_encoded=True)
...
def _png_embedded_xml(data: bytes) -> str | None:
    ...
    while pos + 8 <= len(data):
        ...
        return _decode_utf8(value, url_encoded=True)
    return None
