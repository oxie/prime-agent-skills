...
for(const name of ["ponytail"])assert.equal(loaded.skills.filter(s=>s.name===name).length,1);
console.log(`native loader PASS: ${loaded.skills.length} skills, no diagnostics`);
const fixture=fs.mkdtempSync(path.join(os.tmpdir(),"ponytail-prime-discovery-"));
...
child.stdout.on("data",chunk=>{buffer+=chunk;let at;
 ...
 if(m.id!=="ponytail-discovery")continue;
 try{assert.equal(m.success,true,JSON.stringify(m));
   for(const name of ["ponytail"]){const c=m.data.commands.filter(c=>c.name===`skill:${name}`);assert.equal(c.length,1);
...
child.stdin.write(JSON.stringify({id:"ponytail-discovery",type:"get_commands"})+"\n");
